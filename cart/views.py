...
def add_cart(request, food_id):
  ...
  try:
    ...
    if (cart_item.quantity < cart_item.food.stock):
      cart_item.quantity += 1
    cart_item.save()
  except CartItem.DoesNotExist:
    cart_item = CartItem.objects.create(food=food, quantity=1,cart=cart)
      ...
      try: 
        ...
        for order_item in cart_items:
          oi = OrderItem.objects.create(
              food = order_item.food.name,
              quantity = order_item.quantity,
              price = order_item.food.price,
              order = order_details)
          if voucher != None:
          ...
          foods = Food.objects.get(id=order_item.food.id)
          foods.stock = int(order_item.food.stock - order_item.quantity)
          foods.save()
          order_item.delete()
...
def full_remove(request, food_id):
  cart = Cart.objects.get(cart_id=_cart_id(request))
  food = get_object_or_404(Food, id=food_id)
  cart_item = CartItem.objects.get(food=food, cart=cart)
  cart_item.delete()
